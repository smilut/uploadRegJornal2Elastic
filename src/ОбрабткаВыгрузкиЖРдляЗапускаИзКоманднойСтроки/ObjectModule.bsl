...
Процедура СохранитьДатуНижнейГраницыВыгрузкиЖР(датаГраницыВыгрузки)
	ключ = КлючХранилищаДатыНижнейГраницыВыгрузкиЖР();
...
Функция ИнициироватьПериодВыгрузки(знач период = 3600)
	...
	датаОкончания = Макс(ТекущаяДата()-1, датаНачала+1); //предварительная дата окончания
	//т.к. нет возможности учитывать мили и микро секунды,
	//отступаем на одну сек. в прошлое, что бы все логи событий 
	//на данный момент были записаны.
	//Это страховка, т.к. неизвестно событие в момент гг.мм.дд чч:мм:сс:035
	//будет записано в ЖР с датой гг.мм.дд чч:мм:сс+1 или гг.мм.дд чч:мм:сс?
	
	//блокируем отрицательный и нулевой периоды
	Если период <= 0 Тогда
		период = 3600;
	КонецЕсли;
	датаОкончания = Мин(датаОкончания, датаНачала+период);
...
Функция СтруктураНастроекДоступаКElastic() Экспорт
	Возврат Новый Структура("АдресСервера, ПортСервера, Сервис, Дебаг, Таймаут, Период", "", 0, "", Ложь, 20, 3600);
КонецФункции
